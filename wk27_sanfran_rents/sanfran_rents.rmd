---
title: "R geospatial SanFran Rents"
...
```{r}
log_file <<- "wk27-sanfran-rents/log.txt"
...
nhoods_shape <- read_sf(here::here("wk27_sanfran_rents/geo_export_3b286e07-2c69-479a-8452-b9acb0b14cfa.shp"))
counties_shape <- read_sf(here::here("wk27_sanfran_rents/CA_counties/CA_Counties_TIGER2016.shp"))
...
g_cali <- ggplot() +
  ...
  geom_text(aes(x, y, label = county), df_counties, family = ft_text, size = 5, colour = txt_col, hjust = 0) +
  geom_text(aes(x, y-0.05, label = label, colour = price), df_counties |> bind_rows(tibble(price = c(327, 10000))), family = ft_text, size = 3, hjust = 0, fontface = "bold") +
  ...
  annotate("text", x = -125.2, y = 39.1, label = "Median rental price", colour = txt_col, size = 12, family = ft_text, hjust = 0) +

  # theme and scales and labs
  coord_sf(default_crs = sf::st_crs(4326), clip = "off") +   # adjust to spatial map scale
  scale_fill_scico(
  ...
  theme(
    ...
    legend.title = element_text(family = ft_text, size = 25, lineheight = 1, face = "bold"),
    legend.text = element_text(family = ft_text, size = 20, face = "bold"),
...
g_sanfran <- ggplot() +
  ...
  geom_ellipse(aes(x0 = x0, y0 = y0, a = a, b = b, angle = 0), df_circle, fill = "grey30", colour = txt_col, alpha = 0.5) +
  geom_tile(aes(lon, lat, fill = price), df_base_sf |> bind_rows(tibble(price = c(327, 10000))), colour = "black") +
...
cali_rental <- g_cali +
  ...
  plot_annotation(
    ...
    theme = theme(
      text = element_text(colour = txt_col),
      plot.background = element_rect(fill = bg, colour = bg),
      plot.title = element_text(hjust = 0.5, family = ft_title, size = 70, face = "bold"),
      plot.subtitle = element_markdown(hjust = 0.5, family = ft_text, size = 25, lineheight = 0.35),
      #plot.caption = element_markdown(hjust = 0.5, family = ft_text, size = 36, lineheight = 0.35, margin = margin(t = 20)),
      legend.title = element_text(family = ft_text, size = 15, lineheight = 0.25, face = "bold"),
      legend.text = element_text(family = ft_text, size = 10, face = "bold"),
      plot.margin = margin(t = 20, r = 20, b = 20, l = 20),
    )
  )

ggsave("sanfran.png", plot = cali_rental, height = 12, width = 20.8)
