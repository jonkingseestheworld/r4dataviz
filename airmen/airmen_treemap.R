...
# created by HecVini
# https://github.com/HecVini/TidyTuesday/blob/main/week6.R


  
us_states <- tibble(state.abb,state.region) %>% 
  clean_names() %>% 
  set_names(c('state','region')) 
...
airmen = readr::read_csv(paste0(tidytuesday_url,'/2022/2022-02-08/airmen.csv')) %>% tibble() %>% clean_names()

airmen <- airmen %>% subset(select = c(graduation_date,pilot_type,state)) %>% drop_na() #select desired data
